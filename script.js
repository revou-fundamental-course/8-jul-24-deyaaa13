...
form.addEventListener('submit', function(event) {
  ...
  if (isNaN(tinggi) || isNaN(berat)) {
    alert("Masukan tinggi dan berat badan harus berupa angka!");
    return;
  }

  if (tinggi <= 0 || berat <= 0) {
    alert("Tinggi dan berat badan tidak boleh 0!");
    return;
  }
  ...
  tampilkanHasilBMI(bmi, jenisKelamin);
});

function tampilkanHasilBMI(bmi, jenisKelamin) {
